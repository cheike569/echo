...
export default class RecordingProcessor {
    ...
    async process() {
        ...
        const waveforms = channelData.map((channel, index) => this.generateWaveformImage(channel, index, sampleRate));
        // Determine overlap
    ...
    generateWaveformImage(channel, index, sampleRate) {
    ...
    detectOverlap(channelData) {
        const threshold = 0.01; // Threshold for silence detection
        const activeRegions = channelData.map((channel) => channel.map((sample) => Math.abs(sample) > threshold));
    ...
    detectLongestPause(channelData, sampleRate) {
        const threshold = 0.01; // Threshold for silence detection
        let longestPause = 0;
        let currentPause = 0;
...
//# sourceMappingURL=recordingProcessor.js.map