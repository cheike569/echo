...
type TranscriptionCallback = (transcription: any) => void;

let callbacks = []
let interimCallbacks = []

export function registerTranscriptCallback(callback: TranscriptionCallback) {
    callbacks.push(callback);
}

export function registerInterimTranscriptCallback(callback: TranscriptionCallback) {
    interimCallbacks.push(callback);
}

export function initializeDeepgram() {
    return new Promise<ListenLiveClient>((resolve) => {
